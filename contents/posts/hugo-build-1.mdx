...
**注意：** 这种安装方式可能安装的不是最新版本的hugo，如果要使用最新版的hugo，请使用其他方式安装。

## 配置文件格式

作为一个小白，配置文件格式是选yaml、toml、json就成为了我第一个疑问。

**TOML**

1. 区分大小写
2. 文件只能包含UTF-8编码的Unicode字符
3. 对缩进不敏感
...
任何格式都支持Strings，但JSON不支持多行字符串，还不支持注释。
...
1. `:=` **的含义**
    1. 此符号`:=`是Python语言中的赋值运算符(主要称为海象运算符)，海象操作符压缩了我们的代码以使其更短。
...
7. `Shortcode`可以简短代码，写一些方法。类似`function`
    
    具体参照写法： https://www.letswrite.tw/hugo-basic/
